...
# fit$save_object(here::here('results', "linear_postBooster_fit.rds"))

#read the fitted model
linear_postBooster_fit <- rio::import(here::here('results', 'linear_postBooster_fit.rds'))
...
draws <- linear_postBooster_fit$draws(variables = c(diag_pars, sero_pars), format = "draws_array")
...
linear_postBooster_fit$diagnostic_summary()

#trace plots
bayesplot::color_scheme_set("brightblue")
trace_plot <- mcmc_trace(draws, pars = diag_pars, facet_args = list(ncol = 1))
trace_plot
ggsave(here::here('output', 'postBooster', "lm_trace_hypers.png"), trace_plot, width = 10, height = 10, dpi = 150)

dens_plot <- mcmc_dens_overlay(draws, pars = diag_pars)
dens_plot
ggsave(here::here('output', 'postBooster', "lm_dens_hypers.png"), dens_plot, width = 10, height = 6, dpi = 150)

#density plots
for (par in c("b0", "b1", "b3")) {
  pars_par <- paste0(par, "[", 1:13, "]")

  ggsave(here::here('output', 'postBooster', paste0("lm_trace_", par, ".png")),
         mcmc_trace(draws, pars = pars_par,
                    facet_args = list(ncol = 4)),
         width = 12, height = 8, dpi = 150)

  ggsave(here::here('output', 'postBooster', paste0("lm_dens_", par, ".png")),
...
print(pairs_plot)
ggsave(here::here('output', 'postBooster', "lm_pairs_st1.png"), pairs_plot, width = 8, height = 8, dpi = 150)
...
p_slope <-
  ...
  labs(x = "Serotype", y = "log-log slope b1",
       title = "Posterior serotype-specific slope on log IgG",
       subtitle = "Values below 0 imply lower risk with higher IgG") +
