...
rr <- read_csv(here::here('output', 'postBooster', 'lm_rVE_pooled.csv'), show_col_types = FALSE) %>%
...
write_csv(abs_ve, here::here('output', 'postBooster', sprintf("lm_absolute_VE_ve0_%02d.csv", round(VE0 * 100))))
...
p_absolute <-
  ...
  labs(x = "Serotype", y = "Absolute vaccine efficacy \nagainst colonization (%)", colour = "Vaccine",
       title   = "VE against colonization (linear model), chained from RR estimates",
...
ggsave(here::here('output', 'postBooster', sprintf("lm_absolute_VE_ve0_%02d.png", round(VE0 * 100))), p_absolute, width = 11, height = 6, dpi = 150)
