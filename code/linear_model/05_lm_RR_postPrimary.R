...
#post-primary head-to-head files
comparisons <- list(
...
p_shared <-
  ...
  scale_y_log10() +
  labs(x = "Serotype", y = "Relative risk of colonization", title = "Shared PCV7 serotypes (linear model, post-primary)") +
  base_theme
print(p_shared)
...
p_extra <-
  ...
  scale_y_log10() +
  labs(x = "Serotype", y = "Relative risk of colonization", title = "Additional PCV13 serotypes (1, 3, 5, 6A, 7F, 19A) - linear post-primary") +
  base_theme
print(p_extra)
...
p_combined <-
  p_shared / p_extra +
  plot_annotation(
    title    = "Relative risk of colonization, higher- vs lower-valency PCV (linear, post-primary)",
...
for (cmp in names(comparisons)) {
  ...
  pp <-
    ...
    scale_y_log10() +
    coord_flip() +
    labs(x = NULL, y = "Relative risk of colonization", title = paste0("Per-study RR (linear post-primary): ", cmp)) +
