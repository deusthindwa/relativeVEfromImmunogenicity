...
stan_data <- list(
  ...
  log_gmc_mean  = mean(dat_postPrimary$log_GMC),
  log_gmc_sd    = sqrt(var(dat_postPrimary$log_GMC) * 2), # var*2 ~ prec/2
...
# fit$save_object(here::here('results', "changepoint_postPrimary_fit.rds"))

#read the dataset
postPrimary_fit <- rio::import(here::here('results', 'changepoint_postPrimary_fit.rds'))
...
draws <- postPrimary_fit$draws(variables = c(diag_pars, sero_pars), format = "draws_array")
...
postPrimary_fit$diagnostic_summary()
