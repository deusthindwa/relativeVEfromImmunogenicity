...
rr <- read_csv(here::here('output', 'postBooster','rVE_pooled.csv'), show_col_types = FALSE) %>%
...
write_csv(abs_ve, here::here('output', 'postBooster', sprintf("absolute_VE_ve0_%02d.csv", round(VE0 * 100))))
...
p_absolute <- 
  ...
  labs(x = "Serotype", y = "Absolute vaccine efficacy \nagainst colonization (%)", colour = "Vaccine", title   = "VE against colonization, chained from RR estimates", subtitle = sprintf("Assumed VE(PCV7) = %.0f%% for PCV7 serotypes and 0%% for the six PCV13-only serotypes", 100 * VE0, 100 * VE0)) +
...
p_relative <- 
  abs_ve %>%
  dplyr::filter(in_pcv7 == FALSE) %>% #vaccine !='PCV13'
  dplyr::mutate(pcvind = factor(if_else(in_pcv7 == TRUE, 'PCV7 serotypes', 'PCV13-only serotypes'), levels = c('PCV7 serotypes', 'PCV13-only serotypes'))) %>%
  ggplot(aes(x = serotype, y = median, ymin = lci, ymax = uci, colour = vaccine)) +
  geom_hline(yintercept = 0, linetype = "dashed", colour = "grey50") +
  facet_grid(. ~ pcvind, scales = "free_x", space = "free_x") +
  geom_pointrange(position = position_dodge(width = 0.55), size = 0.4) +
  scale_y_continuous(limits = c(-0.5, 0.3), labels = scales::percent, n.breaks = 6) +
  scale_colour_manual(values = c("PCV10" = "green4", "PCV13" = "steelblue4", "PCV14" = "gray40", "PCV15" = "darkorange2", "PCV20" = "firebrick")) +
...
ggsave(here::here('output', 'postBooster', sprintf("absolute_VE_ve0_%02d.png", round(VE0 * 100))), p_absolute, width = 11, height = 6, dpi = 150)
