...
rr <- read_csv(here::here('output', 'postBooster','cp_RR_pooled.csv'), show_col_types = FALSE) %>%
...
write_csv(abs_ve, here::here('output', 'postBooster', sprintf("cp_abs_rel_VE.csv", round(VE0 * 100))))
...
p_absolute <- 
  ...
  labs(x = "Serotype", y = "Absolute vaccine efficacy \nagainst colonization (%)", colour = "Vaccine", 
       title   = "VE against colonization, chained from RR estimates (change point model, post-booster)", subtitle = sprintf("Assumed VE(PCV7) = %.0f%% for PCV7 serotypes and 0%% for the six PCV13-only serotypes", 100 * VE0, 100 * VE0)) +
...
ggsave(here::here('output', 'postBooster', sprintf("cp_abs_rel_VE.png", round(VE0 * 100))), p_absolute/p_relative, width = 12, height = 8, dpi = 150)
