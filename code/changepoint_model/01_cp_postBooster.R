...
trace_plot
ggsave(here::here('output', 'postBooster', "cp_trace_hypers.png"), trace_plot, width = 10, height = 10, dpi = 150)

dens_plot <- mcmc_dens_overlay(draws, pars = diag_pars)
dens_plot
ggsave(here::here('output', 'postBooster', "cp_dens_hypers.png"), dens_plot, width = 10, height = 6, dpi = 150)

#density plots
for (par in c("b0", "b1", "b3", "cp")) {
  pars_par <- paste0(par, "[", 1:13, "]")
  
  ggsave(here::here('output', 'postBooster', paste0("cp_trace_", par, ".png")),
         mcmc_trace(draws, pars = pars_par,
                    facet_args = list(ncol = 4)),
         width = 12, height = 8, dpi = 150)
  
  ggsave(here::here('output', 'postBooster', paste0("cp_dens_", par, ".png")),
...
print(pairs_plot)
ggsave(here::here('output', 'postBooster', "cp_pairs_st1.png"), pairs_plot, width = 8, height = 8, dpi = 150)
...
p_risk <- 
  ...
  scale_y_log10() +
  labs(x = "log(IgG concentration)", y = "Predicted risk of colonization", colour = "Ethnicity", fill = "Ethnicity", 
       title = "Posterior risk of colonization by serotype (change point model, post-booster)", subtitle = "Median (line) and 95% credible interval (ribbon)") +
  theme_minimal(base_size = 14) +
  theme(strip.background = element_rect(fill = "grey95", colour = NA))

print(p_risk)
ggsave(here::here('output', 'postBooster', "cp_risk_curve.png"), p_risk, width = 12, height = 8, dpi = 150)
...
print(p_cp)
ggsave(here::here('output', 'postBooster', "cp_changepoint.png"), p_cp, width = 8, height = 5, dpi = 150)
