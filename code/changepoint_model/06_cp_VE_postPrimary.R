...
rr <- read_csv(here::here('output', 'postPrimary','cp_RR_pooled.csv'), show_col_types = FALSE) %>%
...
write_csv(abs_ve, here::here('output', 'postPrimary', sprintf("cp_abs_rel_VE.csv", round(VE0 * 100))))
...
p_absolute <- 
  ...
  labs(x = "Serotype", y = "Absolute vaccine efficacy \nagainst colonization (%)", colour = "Vaccine", 
       title = "VE against colonization, chained from RR estimates (change point post-primary)", subtitle = sprintf("Assumed VE(PCV7) = %.0f%% for PCV7 serotypes and 0%% for the six PCV13-only serotypes", 100 * VE0, 100 * VE0)) +
...
p_relative <- 
  ...
  facet_grid(. ~ pcvind, scales = "free_x", space = "free_x") +
  geom_pointrange(position = position_dodge(width = 0.55), size = 0.4) +
  scale_y_continuous(limits = c(-0.6, 0.6), labels = scales::percent, n.breaks = 6) +
  scale_colour_manual(values = c("PCV10" = "green4", "PCV13" = "steelblue4", "PCV14" = "gray40", "PCV15" = "darkorange2", "PCV20" = "firebrick")) +
...
ggsave(here::here('output', 'postPrimary', sprintf("cp_abs_rel_VE.png", round(VE0 * 100))), p_absolute/p_relative, width = 12, height = 8, dpi = 150)
